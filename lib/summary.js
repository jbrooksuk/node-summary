...
var lastContent, lastSentences_dict    // cache
...
function getSortedSentences(paragraph, sentences_dict, n, callback) {
	splitContentToSentences(paragraph, function(sentences) {
		...
		sorted_sentences = _.map(sentence_scores, 'sentence')
...
function getSentencesRanks(content, callback) {
	if (content == lastContent) {		// skip repeated processing, callback with cache instead
		callback(lastSentences_dict)
		return
	}
	splitContentToSentences(content, function(sentences) {
		...
		lastContent = content
		lastSentences_dict = sentences_dict
