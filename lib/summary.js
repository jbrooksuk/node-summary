...
import { onlyGetSentences, splitContentToSentences, splitContentToParagraphs, sentencesIntersection, formatSentence, getBestSentence, getSortedSentences, getSentencesRanks } from "./helpers/summarizer";
...
exports.summarizeFromUrl = function(url, callback) {
	...
	if (isValidUrl(url)) {
		request.get(url, function(error, response, body) {
			...
			return summaryToolContext.summarize(title, content, (err, result, dict) => {
				if (err) {
