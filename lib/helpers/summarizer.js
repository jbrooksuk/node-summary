import { default as Tokenizer } from 'sbd'
import { intersectSafe } from './intersectSafe.js'
import { each, map, range, some, sortBy } from './lodash.js'

export const onlyGetSentences = text => {
  return text
    .split('* ')
...
export const splitContentToSentences = (content, callback) => {
...
export const splitContentToParagraphs = (content, callback) => {
  callback(content.split('\n\n'))
}

export const sentencesIntersection = (sent1, sent2, callback) => {
  let s1 = sent1.split(' ')
  let s2 = sent2.split(' ')
...
export const formatSentence = (sentence, callback) => {
...
export const getBestSentence = (paragraph, sentences_dict, callback) => {
  splitContentToSentences(paragraph, sentences => {
    if (!sentences) return ''
    ...
    for (s in sentences) {
      ...
      formatSentence(sentence, strip_s => {
...
export const getSortedSentences = (paragraph, sentences_dict, n, callback) => {
  splitContentToSentences(paragraph, sentences => {
...
export const getSentencesRanks = (content, callback, sentences_dict) => {
  ...
  splitContentToSentences(content, function (sentences) {
    ...
    each(zeroNRange, function (i) {
      each(zeroNRange, function (j) {
        sentencesIntersection(sentences[i], sentences[j], function (
          err,
          intersection
        ) {
          if (err) throw err
