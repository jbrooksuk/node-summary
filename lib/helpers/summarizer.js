...
const { each, map, range, some, sortBy } = require('lodash')
const { intersectSafe } = require('./intersectSafe')

exports.onlyGetSentences = text => {
  return text
    .split('* ')
...
exports.splitContentToSentences = (content, callback) => {
...
exports.splitContentToParagraphs = (content, callback) => {
  callback(content.split('\n\n'))
}

exports.sentencesIntersection = (sent1, sent2, callback) => {
  let s1 = sent1.split(' ')
  let s2 = sent2.split(' ')
...
exports.getBestSentence = (paragraph, sentences_dict, callback) => {
  exports.splitContentToSentences(paragraph, sentences => {
    ...
    for (s in sentences) {
      ...
      exports.formatSentence(sentence, strip_s => {
...
exports.getSortedSentences = (paragraph, sentences_dict, n, callback) => {
  exports.splitContentToSentences(paragraph, sentences => {
    ...
    sentences.forEach((s, i) => {
      exports.formatSentence(s, strip_s => {
        if (strip_s) {
    ...
    sentence_scores = sortBy(sentence_scores, sentence_score => {
    ...
    sentence_scores = sentence_scores.sort((a, b) => {
      return a.order - b.order
    })

    callback(map(sentence_scores, 'sentence')) // callback with sorted_sentences. map is former .pluck
  })
}

exports.getSentencesRanks = (content, callback, sentences_dict) => {
  ...
  exports.splitContentToSentences(content, function (sentences) {
    ...
    each(zeroNRange, function (i) {
      each(zeroNRange, function (j) {
        exports.sentencesIntersection(sentences[i], sentences[j], function (
          err,
          intersection
        ) {
          if (err) throw err
    ...
    some(zeroNRange, function (i) {
      score = 0
      some(zeroNRange, function (j) {
        if (i !== j) score += values[i][j]
      })

      exports.formatSentence(sentences[i], function (strip_s) {
